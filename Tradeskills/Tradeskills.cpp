	...
	while (itr != tradeskillList.end()) {
		...
		if (Timer::GetCurrentTime2() >= tradeskill->nextUpdateTime) {
			...
			*/
			...
			client->SimpleMessage(CHANNEL_COLOR_YELLOW, "success mod" );
			...
			// Fail
				...
				durability = -50;
				effect = 3;
				client->SimpleMessage(CHANNEL_COLOR_YELLOW, "Failure!");//added for testeing
			}
			// Success
				...
				durability = -10;
				effect = 2;
				client->SimpleMessage(CHANNEL_COLOR_YELLOW, "Success!");//added for testeing
			}
			else {
...
void TradeskillMgr::BeginCrafting(Client* client, vector<int32> components) {
	...
	client->GetPlayer()->UnlockTSSpells(recipe->GetTechnique());
