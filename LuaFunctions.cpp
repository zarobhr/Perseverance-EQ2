...
int EQ2Emu_lua_SpawnSet(lua_State* state){
	...
	//LogWrite(PLAYER__ERROR, 0, "ROBERT", "Lua command setspawn test fix",spawn);
	if(type != 0xFFFFFFFF && value.length() > 0 && spawn)
		commands.SetSpawnCommand(0, spawn, type, value.c_str());
	//client->GetCurrentZone()->ApplySetSpawnCommand(client, spawn, type, strdup(value.c_str()));
	return 0;
}
...
int EQ2Emu_lua_MovementLoopAdd(lua_State* state){
	...
	boolean remove = lua_interface->GetBooleanValue(state, 8);
	if(spawn){
		spawn->AddMovementLocation(x, y, z, speed, delay, function.c_str(),remove);
...
int EQ2Emu_lua_AddQuestSelectableRewardItem(lua_State* state){
	...
	Quest* quest = lua_interface->GetQuest(state);
	if(quest){
		LogWrite(PLAYER__DEBUG, 0, "Player", "adding questreward ");
...
int EQ2Emu_lua_AddQuestStepChat(lua_State* state){
	...
	if(quest){
		...
		while((npc_id = lua_interface->GetInt32Value(state, 7+i))){
			...
			ids->push_back(npc_id);
			i++;
		}                                  //step, QUEST_STEP_TYPE_OBTAIN_ITEM, description, ids, quantity, taskgroup, 0, 0, percentage,0);
...
int EQ2Emu_lua_Interrupt(lua_State* state) 
{
	...
	if ((target && !target->IsEntity()) && !spell)
