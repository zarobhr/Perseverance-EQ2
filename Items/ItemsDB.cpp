...
void WorldDatabase::LoadDataFromRow(DatabaseResult* result, Item* item) 
{
	...
	item->generic_info.tradeskill_classes		= result->GetInt32Str("tradeskill_classes");
#ifdef WIN32
	//item->generic_info.tradeskill_classes = result->GetInt32Str("sell_price");
	//item->generic_info.adventure_classes		= result->GetInt64Str("adventure_classes");
	//item->generic_info.tradeskill_classes		= result->GetInt32Str("tradeskill_classes");
#else
	item->generic_info.adventure_classes		= result->GetInt64Str("adventure_classes");
...
int32 WorldDatabase::LoadHouseItem()
{
	Query query;
	MYSQL_ROW row;
	MYSQL_RES* result = query.RunQuery2(Q_SELECT, "SELECT item_id, rent_reduction, status_rent_reduction, coin_rent_reduction, house_only,model_id,primary_command,secondary_command,location_allowed FROM item_details_house");
	int32 total = 0;
	int32 id = 0;
	...
	if(result)
	{
		while(result && (row = mysql_fetch_row(result)))
		{
			...
			if(item)
			{
				...
				item->houseitem_info->model_id = atoi(row[5]);
				item->houseitem_info->primary_command = atoi(row[6]);
				item->houseitem_info->secondary_command = atoi(row[7]);
				item->houseitem_info->location_allowed = atoi(row[8]);
...
int32 WorldDatabase::LoadBags()
{
	Query query;
	MYSQL_ROW row;
	MYSQL_RES* result = query.RunQuery2(Q_SELECT, "SELECT item_id, num_slots, weight_reduction,bank_only,harvests_only FROM item_details_bag");
	int32 total = 0;
	int32 id = 0;
	if(result)
	{
	...
	if(result)
	{
		...
		while(result && (row = mysql_fetch_row(result)))
		{
			...
			if(master_item)
			{
				...
				item->details.unique_id = strtoul(row[1], NULL, 0);//id
				item->details.slot_id = atoi(row[2]);//slot
