...
bool PlayerItemList::GetFirstMountSlot(sint32* bag_id, sint16* slot) {
...
bool PlayerItemList::HasFreeMountSlot() {
	bool ret = false;
	MPlayerItems.readlock(__FUNCTION__, __LINE__);
	if (items[-8].size() < 40) //40 mounts
		ret = true;
	MPlayerItems.releasereadlock(__FUNCTION__, __LINE__);
	return ret;
}

int8 PlayerItemList::FindFreeMountSlot() {
	int8 ret = 0;
	MPlayerItems.readlock(__FUNCTION__, __LINE__);
	for (int8 i = 0; i < 40; i++) { //40 mounts
		if (items[-8].count(i) == 0) {
			ret = i;
			break;
		}
