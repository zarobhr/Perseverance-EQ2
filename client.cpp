...
void Client::CreateSpawn(string spawntype, int32 modeltype, int32 classtype, int32 level, string name, int32 difficulty, int32 size, Client* client)
{
	Spawn* spawn = 0;
	if (spawntype == "object"){
		spawn = new Object();
	...
	else if (spawntype == "groundspawn"){
		spawn = new GroundSpawn();
		memset(&spawn->appearance, 0, sizeof(spawn->appearance));
	}
	else if (spawntype == "sign"){
		spawn = new Sign();
		memset(&spawn->appearance, 0, sizeof(spawn->appearance));
	}
	...
	spawn->SetModelType(modeltype);
	spawn->SetAdventureClass(classtype);
	spawn->SetLevel(level);
	spawn->SetName(name.c_str());
	if (difficulty && (difficulty >0))
		spawn->SetEncounterLevel(difficulty);
	if (size && (size > 0))
		spawn->size = size;
...
void Client::SendLoginInfo() {
	...
	SendCollectionList();
	SendBiography();
	ClientPacketFunctions::SendCharacterHousingList(this, world.GetAllPlayerHouses(player->GetCharacterID()));
...
void Client::TourHouse(int32 zoneid)
{
	//ZoneServer* house = zone_list.GetByInstanceID(zoneid, 0); //hz->zone_id);
	/*ZoneServer* house = zone_list.Get(zoneid);
	if (house) {
		Zone(house,true);
	}

	ZoneServer* house = zone_list.GetByInstanceID(ph->instance_id, 0); //hz->zone_id);
	if (house) {
		Zone(house, true);
	}*/
	PlayerHouse* ph = world.GetPlayerHouseByUniqueID(zoneid);
	//if (ph) {
		HouseZone* hz = world.GetHouseZone(zoneid);
		if (hz) {
			ZoneServer* house = zone_list.Get(hz->zone_id); //hz->zone_id);
			if (house) {
				Zone(house, true);
		//	}
		}
...
#endif
	switch(opcode){
		case OP_LoginByNumRequestMsg:{ //2
		...
		case OP_QuestJournalOpenMsg: //82
		case OP_QuestJournalInspectMsg:{ //83
		...
		case OP_QuestJournalSetVisibleMsg:{ //84
		...
		case OP_PositionMoveableObject:{ //113
		...
		case OP_EntityVerbsRequestMsg:{
			LogWrite(OPCODE__DEBUG, 0, "Opcode", "Opcode 0x%X (%i): OP_EntityVerbsRequestMsg", opcode, opcode);
			HandleVerbRequest(app);
			break;
									  }
		case OP_EntityVerbsVerbMsg:{
			LogWrite(OPCODE__DEBUG, 0, "Opcode", "Opcode 0x%X (%i): OP_EntityVerbsVerbMsg", opcode, opcode);
		...
		case OP_RemoteCmdMsg:{
			...
			if(app->size > 0){
				...
				LogWrite(PACKET__DEBUG, 0, "Packet", "RemoteCmdMsg Packet dump:");
#if EQDEBUG >= 9
				DumpPacket(app);
#endif
				DumpPacket(app);
				//LogWrite(WORLD__ERROR, 0, "ROBERTTest", "Remote command %i", opcode);
		...
		case OP_EqCollectionUpdateCmd:{
			...
			LogWrite(PLAYER__ERROR, 0, "Player", "OPCODE add collection");
			break;
		}
		case OP_RecipeBook:{
		...
		case OP_BuyPlayerHouseMsg: {
			LogWrite(OPCODE__DEBUG, 0, "Opcode", "Opcode 0x%X (%i): OP_BuyPlayerHouseMsg", opcode, opcode);
			//DumpPacket(app);
			...
			if (packet) {
				...
				if (hz) {
					...
					world.AddPlayerHouse(GetPlayer()->GetCharacterID(), hz->id, unique_id, instance_zone->GetInstanceID(), upkeep_due, 0, 0, GetPlayer()->GetName(),1);
				}
			}
		...
		case OP_EnterHouseMsg: {
			LogWrite(OPCODE__DEBUG, 0, "Opcode", "Opcode 0x%X (%i): OP_EnterHouseMsg", opcode, opcode);
			//DumpPacket(app);
			...
			if (packet) {
				...
				if (ph) {
					HouseZone* hz = world.GetHouseZone(ph->house_id);
					if (hz) {
						ZoneServer* house = zone_list.GetByInstanceID(ph->instance_id, hz->zone_id); //hz->zone_id);
						if (house) {
							LogWrite(OPCODE__DEBUG, 0, "House unique zone: %i", "", house->GetInstanceID())  ;
							Zone(house, true);
						}
					}
				}
		...
		case OP_HouseAccessSetMsg:
		{
			LogWrite(COMMAND__ERROR, 0, "Opcode", "Opcode 0x%X (%i): OP_HouseAccessSetMsg", opcode, opcode);
			PacketStruct* packet = configReader.getStruct("WS_HouseAccessSetMsg", GetVersion());
			if (packet) {
				packet->LoadPacketData(app->pBuffer, app->size);
				int64 house_id = packet->getType_int64_ByName("house_id");
				string player_name = packet->getType_EQ2_16BitString_ByName("player_name").data;
				int8 access_level = packet->getType_int8_ByName("access_level");
				ClientPacketFunctions::SendPlayerHouseAccessUpdate(this, 0, player_name, access_level);
				}
		...
		case OP_HouseDefaultAccessSetMsg:
		{
			LogWrite(COMMAND__ERROR, 0, "Opcode", "Opcode 0x%X (%i): OP_HouseAccessSetMsg", opcode, opcode);
			PacketStruct* packet = configReader.getStruct("WS_HouseDefaultAccessSetMsg", GetVersion());
			if (packet) {
				packet->LoadPacketData(app->pBuffer, app->size);
				int64 house_id = packet->getType_int64_ByName("house_id");
				int8 access_level = packet->getType_int8_ByName("access_level");
				WorldDatabase::SetHouseDefaultAccess(house_id, access_level);
			}
		...
		default:{
			LogWrite(COMMAND__ERROR, 0, "Opcode", "Opcode 0x%X (%i): Unknown in %s", opcode, opcode, __FILE__);
			const char* name = app->GetOpcodeName();
			if(name)
				LogWrite(COMMAND__ERROR, 0, "Opcode", "%s Received %04X (%i)", name, app->GetRawOpcode(), app->GetRawOpcode());
			else
				LogWrite(COMMAND__ERROR, 0, "Opcode", "Received %04X (%i)", app->GetRawOpcode(), app->GetRawOpcode());
...
void Client::HandleExamineInfoRequest(EQApplicationPacket* app){
	...
	LogWrite(WORLD__ERROR, 0, "World", "Robert examine item type: %i", type);
	//DumpPacket(app);
	if(type == 3){
		Spell* spell = 0;
		bool trait_display;
	...
	else if(type == 0){ //merchant buy tab
		...
		request->LoadPacketData(app->pBuffer, app->size);
		int32 id = request->getType_int32_ByName("id");
		//LogWrite(WORLD__ERROR, 0, "World", "Robert examine item id: %i Type: %i", id,type);
		...
		if(item){// && sent_item_details.count(id) == 0){
			...
			//LogWrite(WORLD__ERROR, 0, "World", "Robert dumping item packet");
			//DumpPacket(app);//testpacket
			QueuePacket(app);
		}
		else{
	...
	else if (type == 1){// delete and change 111 back to 1 //merchant Sell/buyback Tab
		...
		//LogWrite(WORLD__ERROR, 0, "World", "Robert examine item id: %i Type: %i", id,type);
		LogWrite(WORLD__ERROR, 0, "ROBERTTest", "ID: %i   Unique: %i", id, unique_id);
		...
		if (item){
			...
			//EQ2Packet* app = item->serialize(GetVersion(), (request->getType_int32_ByName("unique_id") != 0xFFFFFFFF), GetPlayer(), false, 0, 0, true);
			//DumpPacket(app);
			QueuePacket(app);
		}
		else{
	...
	else if (type == 2){// should be 2 testing
		...
		LogWrite(WORLD__ERROR, 0, "ROBERTTest", "ID: %i", id);
		if(item){
			//only display popup for non merchant links
...
bool Client::Process(bool zone_process) {
	...
	while(ret && eqs && (app = eqs->PopPacket())) {
		...
		LogWrite(CCLIENT__DEBUG, 1, "Client", "Func: %s, Line: %i, Opcode: '%s'", __FUNCTION__, __LINE__, app->GetOpcodeName());
		//debug 5 above
		delete app;
	}
	if(quest_updates) {
...
void Client::Zone(int32 instanceid, bool set_coords, bool byInstanceID) {
	...
	SendChangeServerControlFlagMsg(1, GetVersion());
}
...
void Client::Loot(int32 total_coins, vector<Item*>* items, Entity* entity ){
	...
	if(packet){
		...
		int32 packet_size = 0;
		if(items && items->size() > 0){
			LogWrite(PLAYER__DEBUG, 1, "Player", "lootitems: '%i'", items->size());
		...
		LogWrite(PLAYER__DEBUG, 1, "Player", "GetID: '%i'", entity->GetID());
		...
		if(items && items->size() > 0){
			for(itr = items->begin(); itr != items->end(); itr++){
				...
				LogWrite(PLAYER__DEBUG, 1, "Player", "loot item id: '%i'", &item->details.item_id);
				LogWrite(PLAYER__DEBUG, 1, "Player", "Entity: '%i'", entity->GetID());
				tmpPacket = item->serialize(GetVersion(), true, GetPlayer(), false, 1, 0, false, true);

				int8 offset = 0;
				if ((GetVersion() >= 63119) || GetVersion() == 61331){
					offset = 13;
...
void Client::SendChangeServerControlFlagMsg(boolean flag, int16 version) {
	PacketStruct* packet = configReader.getStruct("WS_ServerControlFlags", version);
	if (packet) {
		packet->setDataByName("parameter5", 64);
		packet->setDataByName("value", flag);
		//QueuePacket(packet->serialize());
	}
	...
	if(packet){
		...
		if (items2 && items2->size() > 0){
			...
			for (int32 i = 0; i < items2->size(); i++){
				...
				item = items2->at(i);
				uint64 pc = pow(2, player->GetAdventureClass() - 1);
				uint64 tc = pow(2, player->GetTradeskillClass() - 1);
				if (((item->generic_info.adventure_classes == 0) & (item->generic_info.tradeskill_classes == 0)) ||
					((pc & (item->generic_info.adventure_classes)) > 0) ||
					((tc & (item->generic_info.tradeskill_classes)) > 0)){
					j += 1;
				}
				else j += 1; // comment out for class specific quest rewards
				
			}
			packet->setArrayLengthByName("num_select_rewards", j);
			j = 0;
			for (int32 i = 0; i < items2->size(); i++){
				item = items2->at(i);
				int64 pc = pow(2, player->GetAdventureClass() - 1);
				int64 tc = pow(2, player->GetTradeskillClass() - 1);
				LogWrite(PLAYER__DEBUG, 0, "Player", "J plater ADV class: '%i' ", (pc & (item->generic_info.adventure_classes/2)));
				LogWrite(PLAYER__DEBUG, 0, "Player", "J Player trasde cklass: '%i' ", (tc & (item->generic_info.tradeskill_classes/2)));
				if (((item->generic_info.adventure_classes == 0) & (item->generic_info.tradeskill_classes == 0)) ||
					((pc & (item->generic_info.adventure_classes /2)) > 0) ||
					((tc & (item->generic_info.tradeskill_classes /2)) > 0) || (0 == 0)) { // adde 0 == 0 until we figure a way for choosing whether we want class specific rewar
					j += 1;
					LogWrite(PLAYER__DEBUG, 0, "Player", "J at quest turn in: '%i' ", j);
					packet->setArrayDataByName("select_reward_id", items2->at(i)->details.item_id, j -1);
					LogWrite(PLAYER__DEBUG, 0, "Player", "Quest reward name: %s  '%i' ", items2->at(i)->name , j);
					if (version < 860)
						packet->setItemArrayDataByName("select_item", items2->at(i), player, j - 1, 0, -1);
					else if (version < 1193)
						packet->setItemArrayDataByName("select_item", items2->at(i), player, j - 1);
					else
						packet->setItemArrayDataByName("select_item", items2->at(i), player, j - 1, 0, 2);
				}
			}
		}
		map<int32, sint32>* reward_factions = quest->GetRewardFactions();
		...
		if(items){
			...
			if(packet){
				...
				for(itr = items->begin(); itr != items->end(); itr++, i++){
					...
					if (GetVersion() <= 1096)
					packet->setArrayDataByName("description", item->description.c_str(), i);// was commented out, uncommented to test
					else if ((GetVersion() >= 63119) || GetVersion() == 61331){
		...
		if(packet){
			...
			for(itr = buy_back_items.begin(); itr != buy_back_items.end(); itr++, i++){
				...
				if(master_item){
					...
					if (GetVersion() <= 1096)
						packet->setArrayDataByName("description", master_item->description.c_str(), i);// was commented out, uncommented to test
					else if ((GetVersion() >= 63119) || GetVersion() == 61331){
...
bool Client::SendCollectionsForItem(Item *item, int32 item_index) {
	...
	if (collections_to_send.size() > 0) {
		...
		packet->setDataByName("item_id", item->details.item_id);
		packet->setDataByName("unknown3", item_index);//player->GetCollectionList()->Size()
		packet->setDataByName("unknown4", 0);
		QueuePacket(packet->serialize());
		safe_delete(packet);
	}
...
void Client::HandleCollectionAddItem(int32 collection_id, Item *item) {
	...
	PacketStruct *packet;
	char tmp[200] = {0};
	LogWrite(COLLECTION__ERROR, 0, "Collect", "Player Pressed ADD");
	assert(item);
	...
	if ((collection = player->GetCollectionList()->GetCollection(collection_id))) {
		LogWrite(COLLECTION__ERROR, 0, "Collect", "1");
		...
		LogWrite(COLLECTION__ERROR, 0, "Collect", "2");
		/* get the item struct that represents the item for this collection */
		if (!(collection_item = collection->GetCollectionItemByItemID(item->details.item_id))) {
			SendCollectionList();
			LogWrite(COLLECTION__ERROR, 0, "Collect", "Error in Client::HandleCollectionAddItem. Could not find item '%s' required by collection '%s'", item->name.c_str(), collection->GetName());
