...
void Client::Loot(int32 total_coins, vector<Item*>* items, Entity* entity){
	...
	if(packet){
		...
		int32 packet_size = 0;
		if(items && items->size() > 0){
			LogWrite(PLAYER__DEBUG, 0, "Player", "lootitems: '%i'", items->size());
		...
		LogWrite(PLAYER__DEBUG, 0, "Player", "GetID: '%i'", entity->GetID());
		...
		if(items && items->size() > 0){
			for(itr = items->begin(); itr != items->end(); itr++){
				...
				LogWrite(PLAYER__DEBUG, 0, "Player", "loot item id: '%i'", &item->details.item_id);
				LogWrite(PLAYER__DEBUG, 0, "Player", "Entity: '%i'", entity->GetID());
		...
		if (items2 && items2->size() > 0){
			...
			for (int32 i = 0; i < items2->size(); i++){
				...
				if (((item->generic_info.adventure_classes == 0) & (item->generic_info.tradeskill_classes == 0)) ||
					...
					((tc & (item->generic_info.tradeskill_classes /2)) > 0)){
					j += 1;
		...
		if(items){
			...
			if(packet){
				...
				for(itr = sellable_items.begin(); itr != sellable_items.end(); itr++, i++){
					...
					packet->setArrayDataByName("item_name", item->name.c_str(), i);
					packet->setArrayDataByName("price", sell_price, i);
					packet->setArrayDataByName("status", 100, i);//additive to status 2 maybe for server bonus etc
					packet->setArrayDataByName("status2", 99, i); //this one is the main status
					packet->setArrayDataByName("item_id", item->details.item_id, i);
