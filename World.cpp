	...
	if (result && mysql_num_rows(result) > 0)
	{
		while (result && (row = mysql_fetch_row(result)))
		{
			...
			//LogWrite(SKILL__DEBUG, 5, "Blue Stats", "Loading SkillID: %u, cur_val: %i, max_val: %l", strtoul(row[0], NULL, 0), atoi(row[1]), atoi(row[2]));
			 ...
			 if((atoi(row[0]) >= 63119)||(atoi(row[0])==61331)) { //KA
...
void World::AddPlayerHouse(int32 char_id, int32 house_id, int64 unique_id, int32 instance_id, int32 upkeep_due, int64 escrow_coins, int32 escrow_status, string player_name, int8 public_access_level) {
	...
	if (m_playerHouses.count(house_id) == 0 || m_playerHouses[house_id].count(char_id) == 0) {
		...
		ph->public_access_level = public_access_level;

		m_playerHouses[house_id][char_id] = ph;
	}
	MPlayerHouses.releasewritelock(__FUNCTION__, __LINE__);
}
void World::AddHouseItem(int32 id, int32 house_id, int32 item_id, float x, float z, float y, float rotation, float pitch, float roll, float scale, boolean in_crate ) {
	MHouseItems.writelock(__FUNCTION__, __LINE__);
	if (m_houseItems.count(house_id) == 0 || m_playerHouses[house_id].count(item_id) == 0) {
		HouseItems* hi = new HouseItems;
		hi->id = id;
		hi->house_id = house_id;
		hi->item_id = item_id;
		hi->x = x;
		hi->z = z;
		hi->y = y;
		...
		hi->roll = roll;
		hi->scale = scale;
		hi->in_crate = in_crate;

		m_houseItems[id] = hi;
	}
	MHouseItems.releasewritelock(__FUNCTION__, __LINE__);
}
