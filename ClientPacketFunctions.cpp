...
void ClientPacketFunctions::SendLoginAccepted ( Client* client ){
	...
	if(response_packet){
		...
		response_packet->setDataByName("unknown05", -1209532421);//-959971393
...
void ClientPacketFunctions::SendCharacterSheet ( Client* client ){
	...
	if (client->GetVersion() >= 70000) {
		EQ2Packet* app2 = client->GetPlayer()->GetPlayerInfo()->serializeEquipSet(client->GetVersion());
		if (app2)
			client->QueuePacket(app2);
	}
}
