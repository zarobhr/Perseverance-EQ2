...
EQ2Packet * Recipe::SerializeRecipe(Client *client, Recipe *recipe, bool display, int8 packet_type, int8 subpacket_type, const char *struct_name){
	...
	if(item) {
		...
		packet->setSubstructDataByName("recipe_info", "product_name", item->name); //item->name);
	...
	item = master_item_list.GetItemByName(recipe->GetName());
//	packet->setArrayDataByName("build_comp_qty_avail", item->details.count, 0);
	...
	//	packet->setArrayDataByName("build_comp_qty_avail", 16, 2);
	}
	if (recipe->GetBuild4ComponentQuantity() > 0)
	{
		//packet->setArrayDataByName("build_comp", recipe->GetBuild4ComponentTitle(), 3);
		//packet->setArrayDataByName("build_comp_qty", recipe->GetBuild4ComponentQuantity(), 3);
	//	packet->setArrayDataByName("build_comp_qty_avail", 16, 2);
	}
	int32 fuelcount = 0;
...
//	packet->setSubstructDataByName("recipe_info", "fuel_comp_qty", recipe->GetFuelComponentQuantity());
