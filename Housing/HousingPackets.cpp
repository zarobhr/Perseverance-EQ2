...
void ClientPacketFunctions::SendHousePurchace(Client* client, HouseZone* hz, int32 spawnID) {
	...
	if (packet) {
		...
		packet->setDataByName("enable_buy", 1); //enable_buy ? 1 : 0
...
void ClientPacketFunctions::SendBaseHouseWindow(Client* client, HouseZone* hz, PlayerHouse* ph, int32 spawnID) {
	...
	if (packet) {
		...
		packet->setDataByName("spawn_id", spawnID);
		packet->setDataByName("unknown00", 203);
		string name;
		name = ph->player_name;
		name.append("'s ");
		name.append(hz->name);
		...
		if (client->GetCurrentZone()->GetInstanceType() == PERSONAL_HOUSE_INSTANCE
			...
			packet->setDataByName("public_access_level", ph->public_access_level);
			packet->setDataByName("num_history", 0);
			packet->setDataByName("unknown8", 0); //prevent house windows from poping up
		}
		//packet->setDataByName("unknown6b", 0);
		client->QueuePacket(packet->serialize());
	}
...
void ClientPacketFunctions::SendHouseItemsList(Client* client, vector<PlayerHouse*> houses) {
	PacketStruct* packet = configReader.getStruct("WS_DisplayVisitScreen", client->GetVersion());
	if (packet) {
		vector<PlayerHouse*>::iterator itr;
		packet->setArrayLengthByName("num_houses", houses.size());
...
void ClientPacketFunctions::SendPlayerHouseAccessUpdate(Client* client, bool access_delete,string access_name,int8 access_level) {
	PacketStruct* packet = configReader.getStruct("WS_PlayerHouseAccessUpdateMsg", client->GetVersion());

	if (packet) {
		
		packet->setDataByName("unique_id", database.GetCharacterID(access_name.c_str()));
		packet->setDataByName("access_name", access_delete);
		packet->setDataByName("access_name", access_name.c_str());
		packet->setDataByName("access_level", access_level);
		packet->PrintPacket();
...
void ClientPacketFunctions::SendCharacterHousingList(Client* client, vector<PlayerHouse*> houses) {
	PacketStruct* packet = configReader.getStruct("WS_CharacterHousingList", client->GetVersion());
	if (packet) {
		vector<PlayerHouse*>::iterator itr;
		packet->setArrayLengthByName("num_houses", houses.size());//houses.size()
		int16 i = 0;
		for (itr = houses.begin(); itr != houses.end(); itr++) {
			PlayerHouse* ph = *itr;
			HouseZone* hz = world.GetHouseZone(ph->house_id);
			if (ph && hz) {
				packet->setArrayDataByName("house_id", ph->unique_id, i);
				packet->setArrayDataByName("unknown1",ph->house_id, i);
				packet->setArrayDataByName("zone","qeynos_combined01" , i);
				packet->setArrayDataByName("house_City", "Qeynos Capitol District", i); //hz->name.c_str()
				packet->setArrayDataByName("house_address",":a6f00aebb6b82196:2 Lucie Street" , i);//client->GetCurrentZone()->GetZoneName()
				packet->setArrayDataByName("house_description", "Emem's House", i);//client->GetCurrentZone()->GetZoneName()
				packet->setArrayDataByName("unknown2a", "247507750", i);
				packet->setArrayDataByName("index", i+1, i);
				i++;
			}
		}
		//client->QueuePacket(packet->serialize());
...
/*
void ClientPacketFunctions::EnterMoveObjectModeMsg(Client* client, vector<PlayerHouse*> houses) {
}
void ClientPacketFunctions::MoveableObjectPlacementCriteri(Client* client, vector<PlayerHouse*> houses) {
}
void ClientPacketFunctions::PositionMoveableObject(Client* client, vector<PlayerHouse*> houses) {
}
void ClientPacketFunctions::CancelMoveObjectModeMsg(Client* client, vector<PlayerHouse*> houses) {
}*/
/*
