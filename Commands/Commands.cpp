	...
	if(temporary && temp_value){
		...
		switch(type){
			...
			case SPAWN_SET_VALUE_SHOW_NAME:{
				LogWrite(PLAYER__ERROR, 0, "ROBERT", "Lua command setspawn case 1");
		...
		switch(type){
			...
			case SPAWN_SET_VALUE_SHOW_NAME:{
				LogWrite(PLAYER__ERROR, 0, "ROBERT", "Lua command setspawn case 2");
				target->SetShowName(val, send_update);
				break;
										   }
			case SPAWN_SET_VALUE_ATTACKABLE:{
...
void Commands::Process(int32 index, EQ2_16BitString* command_parms, Client* client){
	...
	LogWrite(COLLECTION__ERROR, 0, "Collect", "Robert trying for a collection command");
	...
	if(command_parms->size > 0){
		...
		if(sep && sep->arg[0] && remote_commands->validSubCommand(command->command.data, string(sep->arg[0]))){
			...
			LogWrite(COMMAND__DEBUG, 0, "Command", "Handler: %u, COMMAND: '%s', SUBCOMMAND: '%s'", index, parent_command->command.data.c_str(), command->command.data.c_str());
		}
		else
			LogWrite(COMMAND__DEBUG, 0, "Command", "Handler: %u, COMMAND: '%s'", index, command->command.data.c_str());
		//above 2 debugs 1
	}
	...
	switch(command->handler){
		case COMMAND_RELOAD:{
			...
			client->SimpleMessage(CHANNEL_COLOR_YELLOW, "/reload spawn");
		...
		case COMMAND_RELOAD_SPAWNS:{
			...
			break;                 }
		case COMMAND_RELOAD_SPAWN:{
			client->SimpleMessage(CHANNEL_COLOR_YELLOW, "You just created the Reload SPan Command!");
			break;				   }
		...
		case COMMAND_INFO:{
			if(sep && sep->arg[1][0] && sep->IsNumber(1)){
				if(strcmp(sep->arg[0], "inventory") == 0){
					...
					if(item){
						if (item->IsCollectable() && client->SendCollectionsForItem(item,item_index))
							break;
		...
		case COMMAND_PLACE_HOUSE_ITEM: {
			client->CreateSpawn("object", 6, 0, 0, "mirror", 0, 0, client);

			break;
		}
		case COMMAND_HOUSE_UI: {
			LogWrite(ITEM__DEBUG, 0, "Items", "HOUSEID:");
			break;
		}
		...
		case COMMAND_COLLECTION_ADDITEM: {
			...
			if (sep && sep->arg[0] && sep->arg[1] && sep->IsNumber(0) && sep->IsNumber(1)) {
				...
				LogWrite(COLLECTION__ERROR, 0, "Collect", "Error in 'COMMAND_COLLECTION_ADDITEM'. item number typed is '%s' at index %u", client->GetPlayer()->GetName(), atoul(sep->arg[1]));
		...
		case COMMAND_SPAWN_MOVE:{
			...
			if(target && target->IsPlayer() == false){
				...
				if(packet){
					...
					packet->setDataByName("unknown4a", 23);
					packet->setDataByName("model_type2", target->GetModelType());
					packet->setDataByName("unknown5", 0x0000FFFF,0);
					packet->setDataByName("unknown5", 0xFFFFFFFF, 1);
					packet->setDataByName("unknown5", 0xFF00FF00, 2);
					packet->setDataByName("unknown8", 2);
					packet->setDataByName("unknown9", 2);
					packet->setDataByName("model_type3", target->GetModelType());
					packet->PrintPacket();
					client->QueuePacket(packet->serialize());
					safe_delete(packet);
				}
		...
		case COMMAND_STATE:{
			...
			if (spawn && spawn->IsEntity() && sep && sep->arg[0] && sep->IsNumber(0)){
				...
				int32 value1 = spawn->GetID();
				int32 value2 = atoul(sep->arg[0]);
				PacketStruct* packet = configReader.getStruct("WS_StateCmd", client->GetVersion());
				if (packet){
					packet->setDataByName("spawn_id", 27010);
					packet->setDataByName("state", value2);
				...
				client->SimpleMessage(CHANNEL_COLOR_YELLOW, "Syntax: /state {state}");
			break;
		}
		...
		case COMMAND_SPAWN_DETAILS:{
			...
			if (spawn){
				...
				if (packet) {
					...
					details3 += "Spawn Expires " + to_string(spawn->GetExpireTime()) + "\n";
		...
		case COMMAND_SPAWN_SET:
			...
			sint16 set_type = -1; 
			string type_str;
			LogWrite(PLAYER__ERROR, 0, "ROBERT", "command_spawn_set");
			if (spawn)
			{
		...
		case COMMAND_HOUSE: {
			PlayerHouse* ph = world.GetPlayerHouseByInstanceID(player->GetZone()->GetInstanceID());
			if (ph) {
...
void Commands::Command_Inventory(Client* client, Seperator* sep, EQ2_RemoteCommandString* command)
{
	...
	if(sep && sep->arg[0][0])
	{
		...
		else if(sep->arg[4][0] && strncasecmp("move", sep->arg[0], 4) == 0 && sep->IsNumber(1) && sep->IsNumber(2) && sep->IsNumber(3) && sep->IsNumber(4))
		{
			...
			if (bag_id == -1 )
			{
					client->SimpleMessage(CHANNEL_COLOR_RED, "You are sending to bag -1");
					return;
			}
			if (bag_id == -2)
			{
				client->SimpleMessage(CHANNEL_COLOR_RED, "You are sending to bag -2");
				return;
			}
			if (bag_id == -5)
			{
				client->SimpleMessage(CHANNEL_COLOR_RED, "You are sending to bag -5");
				return;
			}
			if (bag_id == -6)
			{
				client->SimpleMessage(CHANNEL_COLOR_RED, "You are sending to bag -6");
				return;
			}
			if (bag_id == -7)
			{
				client->SimpleMessage(CHANNEL_COLOR_RED, "You are sending to bag -7");
				return;
			}
			if (bag_id == -8)
			{
				client->SimpleMessage(CHANNEL_COLOR_RED, "You are sending to bag -8");
				return;
			}
		...
		else if (sep->arg[3][0] && strncasecmp("move", sep->arg[0], 4) == 0 && sep->IsNumber(1) && sep->IsNumber(2) && sep->IsNumber(3) )
		{
			int16 from_index = atoi(sep->arg[1]);
			sint16 to_slot = atoi(sep->arg[2]);
			sint32 bag_id = atol(sep->arg[3]);
			...
			if (bag_id == -8 && to_slot == -1)
			{
				if (!player->item_list.GetFirstMountSlot(&bag_id, &to_slot))
				{
					client->SimpleMessage(CHANNEL_COLOR_RED, "You do not have any free slots.");
					return;
				}
			}
			EQ2Packet* outapp = client->GetPlayer()->MoveInventoryItem(bag_id, from_index, (int8)to_slot, 1, client->GetVersion());
			client->QueuePacket(outapp);
...
void Commands::Command_Test(Client* client, EQ2_16BitString* command_parms) {
	...
	//if (sep2 && sep2->arg[0] ) {
	//if (sep2 && sep2->arg[0] && sep2->IsNumber(0)) {// for spell failed packet
...
//	}
	...
	///*};
	//uchar blah[] = { 0x00, 0xD9, 0xCD, 0xCC, 0x4C, 0x3E, 0x00 };//shake
	uchar blah[] = { 0x00, 0xFF, 0x95, 0x02, 0x7E, 0xC2, 0x00, 0x00, 0xB7, 0x32, 0x00, 0x00, 0x00, 0x00 };// eqstate
	...
	//client->QueuePacket(new EQ2Packet(OP_PerformCameraShakeMsg , blah, sizeof(blah)));
	client->QueuePacket(new EQ2Packet(OP_EqStateCmd, blah, sizeof(blah)));
}
