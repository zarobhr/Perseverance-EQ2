...
void Spawn::CreateSpawn(string spawntype, int32 modeltype, int32 classtype, int32 level, string name, int32 difficulty, int32 size,Client* client,boolean houseitem=0)
{
	Spawn* spawn = 0;
	if (spawntype== "object"){
		spawn = new Object();
...
void Spawn::InitializePosPacketData(Player* player, PacketStruct* packet){
	...
	else if(IsGroundSpawn() || IsObject()){
	...
	// Movement loop is only for scripted paths
		...
		if(resume_movement){
			...
			if(data->delay == 0 && movement_loop.size() > 0) {
				...
				boolean doiremove2 = data2->remove;
				if (doiremove2 == 1) {
					movement_loop.erase(movement_loop.begin() + movement_index);
				}
			}
		...
		// If we are not moving or we have arrived at our destination
			...
			// If this waypoint has no delay or we have waited the required time (current time >= delay + movement_start_time)
				...
				if(data->delay == 0 && movement_loop.size() > 0){
					...
					AddRunningLocation(data2->x, data2->y, data2->z, data2->speed);
					boolean doiremove2 = data->remove;
					if (doiremove2 == 1) {
						movement_loop.erase(movement_loop.begin() + movement_index);
					}
				}
...
void Spawn::AddMovementLocation(float x, float y, float z, float speed, int16 delay, const char* lua_function,boolean remove){
