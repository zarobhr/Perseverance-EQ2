...
struct PlayerHouse {
	...
	int32 escrow_status;
	string player_name;
	int8 public_access_level;
};
struct HouseItems {
	int32 id;
	int32 house_id;
	int32 item_id;
	int32 unique_item_id;
	float x;
	float z;
	float y;
	float rotation;
...
public:
	...
	void AddPlayerHouse(int32 char_id, int32 house_id, int64 unique_id, int32 instance_id, int32 upkeep_due, int64 escrow_coins, int32 escrow_status, string player_name, int8 public_access_level);
	...
	void AddHouseItem(int32 id, int32 house_id, int32 item_id, float x, float z, float y, float rotation, float pitch, float roll, float scale, boolean in_crate);
private:
	...
	map<int32, HouseItems*> m_houseItems;
	Mutex MHouseZones;
	Mutex MPlayerHouses;
	Mutex MHouseItems;
