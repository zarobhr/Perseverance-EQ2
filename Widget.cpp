...
void Widget::HandleUse(Client* client, string command){
	...
	else if (client && m_houseID > 0 && strncasecmp("tour", command.c_str(), 6) == 0) {
		// Used a door to tour a house
		//HouseZone* hz = world.GetHouseZone(m_houseID);
		//int32 id = client->GetPlayer()->GetIDWithPlayerSpawn(this);
		//PlayerHouse* ph = 0;
		//if (hz)
			client->TourHouse(m_houseID);
	}
	else if (client &&  strncasecmp("exit", command.c_str(), 6) == 0) {
		// Used a door to tour a house
		ZoneServer* zz = client->GetCurrentZone();
		HouseZone* hz = world.GetHouseZone(zz->GetZoneID());
		//int32 id = client->GetPlayer()->GetIDWithPlayerSpawn(this);
		PlayerHouse* ph = 0;
		client->ExitHouse();
