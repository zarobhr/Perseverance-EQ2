...
void Spell::SetPacketInformation(PacketStruct* packet, Client* client, bool display_tier){
	...
	packet->setSubstructDataByName("spell_info", "unknown_MJ1a", 1); //63119 test
	...
	//packet->setSubstructDataByName("spell_info", "unknown9", 0);
	int16 hp_req = 0;
	int16 power_req = 0;
	if (client) {
...
EQ2Packet* Spell::SerializeAASpell(Client* client, AltAdvanceData* data, bool display, int16 packet_type, int8 sub_packet_type){
	...
	packet->setSubstructDataByName("spell_info", "version",2);//prior to 63119 =0x04
	packet->setSubstructDataByName("spell_info", "sub_version",23);//prior to 63119 =0x03EA
	packet->setSubstructDataByName("spell_info", "type", spell->type);
	packet->setSubstructDataByName("spell_info", "class_skill", spell->class_skill);
	packet->setSubstructDataByName("spell_info", "mastery_skill", spell->mastery_skill);
	if(client && spell->type != 2){
...
//	sub_packet_type = 0x80;
	...
	//packet->setDataByName("unknown2",5);packet->PrintPacket
...
int8 Spell::GetTSIndex(){
	if (spell)
		return spell->ts_loc_index ;
	return 0;
}
